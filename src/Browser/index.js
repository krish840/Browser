...
class Browser extends Component {
  ...
  deleteHistory = id => {
    ...
    const filtered = HistoryDetails.filter(each => each.id !== id)
    this.setState({HistoryDetails: filtered})
  }
    ...
    return (
      <div>
        ...
        <img
          ...
          alt="search"
        />
        <input onChange={this.onChangeSearchInput} value={searchInput} />
        <ul>
          {searchResults.map(each => (
            <HistoryItem key={each.id} HistoryDetails={each} />
          ))}
        </ul>
